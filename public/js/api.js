...
function getData(url, cidade, key) {
    const request = $.ajax({
        ...
        $.post("/", {
            ...
            const historico = $.ajax({
                url: "/consultaBanco",
                jsonpCallback: "trataHis",
            }).then(function (data) {
                console.log(data)
                trataHis(data);
            })
...
function trataHis(forecast) {
    var html = '';
    for (obj of forecast) {
        html += "<tr><th>" + obj.Nome + "</th><th>" + obj.Temperatura + "°C</th><th>" + obj.Condicao + "</th></tr>"
    }
    document.getElementById("tabHis").innerHTML = html;
    
    var flags = [], output = [];
    for (var i = 0; i < forecast.length; i++) {
        var exist = flags[forecast[i].Nome]
        if (exist) {
            forecast[i].Quantidade += 1;
            continue
        };
